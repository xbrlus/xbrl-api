...
xbrlParts REST API
----
This API allows the user to get all the calculation parts comprising an element across all networks in the filing. The API requires the element name and the filing number. The API will return all calculation children of the specified element plus attributes such as weight, the effective weight, the leaf node, balance type and associated network.
